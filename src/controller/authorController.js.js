...
const createAuthor = async (req, res) => {
  try {
    ...
    if (Object.keys(result).length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "data is required in the body" });
    if (!fname) return res.status(400).send({ msg: "fname is mandatory" });
    if (!lname) return res.status(400).send({ msg: "lname is mandatory" });
    if (!title) return res.status(400).send({ msg: "title is mandatory" });
    if (!email) return res.status(400).send({ msg: "email is mandatory" });
    if (!password)
    ...
    if (!emumTitle.includes(title))
      return res
        .status(400)
        .send({ status: false, msg: "Title should be of Mr,or Mrs, or Miss" });

    //valid-Email
    if (!validEmail.validate(email))
      return res
        .status(400)
        .send({ status: false, msg: "Enter a valid email" });
    ...
    if (emailId)
      return res
        .status(400)
        .send({ msg: "This emailId is already registered please SignIn" });
...
const loginAuthor = async (req, res) => {
  try {
    ...
    if (!email) return res.status(400).send({ msg: "email id is mandatory" });
